...
use mongodb::{options::FindOptions, Cursor, results};
...
pub async fn insert_one(
    ...
    let db: mongodb::Database = client.database(&params.0);
    let collection: mongodb::Collection<Document> = db.collection::<Document>(&params.1);
    let data: &Value = &body.0;
    let insert: Result<results::InsertOneResult, mongodb::error::Error> = collection.insert_one(data_to_be_inserted, None).await;
    match insert {
        Ok(insert) => {
            HttpResponse::Ok();
        }
